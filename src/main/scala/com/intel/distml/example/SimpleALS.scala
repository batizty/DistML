...
object SimpleALSObject {
  ...
  //YtY: Broadcast[Any]
    ...
    productOutLinks.flatMap {
      case (bid, outLinkBlock) =>
  ...
  def makeInLinkBlock(numBlocks: Int, ratings: Array[Rating]): InLinkBlock = {
    ...
    for (productBlock <- 0 until numBlocks) {
      ...
      ratingsForBlock(productBlock) = groupRatings.map {
        case (p, rs) =>
  ...
  def makeLinkRDDs(numBlocks: Int, ratings: RDD[(Int, Rating)]): (RDD[(Int, InLinkBlock)], RDD[(Int, OutLinkBlock)]) = {

    val links = ratings.groupByKey(new HashPartitioner(numBlocks)).map {
      case (blockid, iter) => {
        //makeInLinkBlock
